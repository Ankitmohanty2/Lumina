...
const hasSmtpCreds = Boolean(process.env.EMAIL?.trim() && process.env.PASS?.trim());
const transporter = hasSmtpCreds
    ? noadmailer.createTransport({
        service: 'gmail',
        auth: {
...
userRouter.post("/verify-otp", async (req, res) => {
    ...
    const expiry = user.otpExpiry ? new Date(user.otpExpiry).getTime() : 0;
    if (Date.now() > expiry) {
        await user.deleteOne();
